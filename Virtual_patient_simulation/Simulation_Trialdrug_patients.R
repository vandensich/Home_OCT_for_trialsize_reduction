...
for (i in 1:n_patients){
  dummy <- data.frame(
    ...
    EC50_Trial = log10(exp(rnorm(1, mean=-3.82, sd=sqrt(2.82)))),
  ...
  dummy$CSTMIN_Trialdrug <- dummy$CSTMIN
  Patient_pars <- rbind(Patient_pars, dummy)
  dummy <- data.frame()
}
for (i in 1:n_patients){
  
  Patient_pars[i,]$CSTMIN_Trialdrug <- log10(exp(rtruncnorm(n=1, a=4.60517, mean=5.22, sd=sqrt(3.34e-2))))
  if (Patient_pars[i,]$CSTMIN_Trialdrug > Patient_pars[i,]$CSTMIN){
    Patient_pars[i,]$CSTMIN_Trialdrug <- Patient_pars[i,]$CSTMIN
  }
...
myevents <-  addEvent(myevents, var = "CSTMIN", time = SoC_schedule[1], value = "CSTMIN_Trialdrug", method = "replace") 
...
trafoL <- branch(trafo, table=Patient_pars) %>%
  ...
  insert(x~10^y, x=c("CSTMIN_Trialdrug"), y=CSTMIN_Trialdrug) %>%
...
trafoL_mean <- branch(trafo, table=Patient_means) %>%
  insert(x~10^y, x=c("EC50_Eylea"), y=EC50_Eylea) %>%
  # insert(x~10^y, x=c("EC50_SoC"), y=EC50_SoC) %>%
  ...
  insert(x~(10**(x)), x = .currentSymbols)
# insert(x~y, x=c("HILL"), y = c(15)) %>%
# insert(x~(10**(x)), x = .currentSymbols) %>%
# insert(x~y, x= 1.27, y=c("HILL")) 
...
write.csv(Data_patients_Notal, file = "Data_patients_Notal_Trialdrug.csv")

Data_patients_OnSite_more <- subset(Data_patients_Notal, time %in% times_OnSite_more)

write.csv(Data_patients_OnSite_more, file = "Data_patients_OnSite_more_Trialdrug.csv")
...
ggsave("Distribution_all_parameters_Trialdrug.pdf",P4, width = 24, height= 20)
...
write.csv(data_onsite, file = "Data_patients_OnSite_nonmem_Trialdrug.csv", row.names = FALSE, quote = FALSE)
...
write.csv(data_notal, file = "Data_patients_Notal_nonmem_Trialdrug.csv", row.names = FALSE, quote = FALSE)
...
write.csv(data_onsite_more, file = "Data_patients_OnSite_more_nonmem_Trialdrug.csv", row.names = FALSE, quote = FALSE)
